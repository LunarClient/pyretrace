...
class MappingReader:
    def __init__(self, mapping_file, load_file):
    ...
    def pump(self, mapping_processor):
        ...
        if self.load_file:
            ...
            lines = self.mapping_file.splitlines()
        ...
        try:
            ...
            for line in lines:
                ...
                if line.endswith(":"):
                    ...
                    self.process_class_member_mapping(
                        class_name, line, mapping_processor
                    )
        except Exception as ex:
            print(f"Can't process mapping file ({ex})")
    ...
    def process_class_member_mapping(class_name, line, mapping_processor):
        ...
        argument_index2 = (
            -1 if argument_index1 < 0 else line.find(")", argument_index1 + 1)
        )
        ...
        name = line[
            space_index + 1 : argument_index1 if argument_index1 >= 0 else arrow_index
        ].strip()
        new_name = line[arrow_index + 2 : len(line)].strip()

        # Process this class member mapping.
        if len(type) > 0 and len(name) > 0 and len(new_name) > 0:
            # Is it a field or a method?
            if argument_index2 < 0:
                mapping_processor.process_field_mapping(
                    class_name, type, name, new_name
                )
            else:
                first_line_number = 0
                last_line_number = 0
                ...
                if colon_index2 > 0:
                    ...
                    last_line_number = int(
                        line[colon_index1 + 1 : colon_index2].strip()
                    )

                arguments = line[argument_index1 + 1 : argument_index2].strip()

                mapping_processor.process_method_mapping(
                    class_name,
                    first_line_number,
                    last_line_number,
                    type,
