...
class Retrace:
    def __init__(
        ...
        load_file=True,
        verbose=False,
        regular_expression=STACK_TRACE_EXPRESSION,
        ...
        mapping_reader = MappingReader(self.mapping_file, self.load_file)
        ...
        while True:
            ...
            if (
                next_index < 0
                or next_index is (len(self.regular_expression) - 1)
                or self.expression_type_count is len(self.expression_types)
            ):
                break
        ...
        expression_buffer += self.regular_expression[
            index : len(self.regular_expression)
        ]
    ...
    def deobfuscate(self, line, simple_name=False):
        """Return a deobfuscated version of the given line
        :rtype: str
        """

        line = line.replace("at Genesis//", "at ")
            ...
            for expression_type_index in range(0, self.expression_type_count):
                ...
                if start_index >= 0:
                    ...
                    if expression_type == "c":
                        ...
                        class_name = self.original_class_name(
                            external_class_name(match), simple_name
                        )
            ...
            for expression_type_index in range(0, self.expression_type_count):
                ...
                if start_index >= 0:
                    ...
                    if expression_type == "c":
                        ...
                        out_line += self.original_field_name(
                            class_name, match, type, out_line, extra_outlines
                        )
                    elif expression_type == "m":
                        out_line += self.original_method_name(
                            class_name,
                            match,
                            line_number,
                            type,
    ...
    def original_field_name(
        self, class_name, obfuscated_field_name, type, out_line, extra_outlines
    ):
                ...
                for method_info in method_set:
                    if method_info.matches(line_number, type, arguments):
                        ...
                        if extra_indent < 0:
                            ...
                            if self.verbose:
                                original_method_name = (
                                    f"{out_line}({method_info.arguments})"
                                )
                        else:
                            extra_buffer = ""
                            for _ in range(0, extra_indent):
                                ...
                                if self.verbose:
                                    extra_buffer += (
                                        f"{extra_buffer}({method_info.arguments})"
                                    )
    ...
    def original_arguments(self, obfuscated_arguments):
        ...
        while True:
            ...
            original_arguments += (
                self.original_type(obfuscated_arguments[start_index, end_index]) + ","
            )
    ...
    def original_type(self, obfuscated_type):
        ...
        if index >= 0:
            return (
                self.original_class_name(obfuscated_type[0, index])
                + obfuscated_type[index, len(obfuscated_type)]
            )

        return self.original_class_name(obfuscated_type)

    def original_class_name(self, obfuscated_class_name, simple_name=None):
        """
        ...
        if original_class_name:
            if simple_name:
                original_class_name = original_class_name[
                    original_class_name.rfind(".") + 1 : len(original_class_name)
                ]
...
class MethodInfo:
    ...
    def __init__(
        self, first_line_number, last_line_number, type, arguments, original_name
    ):
        ...
        return (
            (
                ...
                or (
                    ...
                    and line_number <= self.last_line_number
                )
                or self.last_line_number == 0
            )
            and (type is None or type == self.type)
            and (arguments is None or arguments == self.arguments)
        )
